...
def load_mnist(data_path, validation_size, source_url, one_hot):
    ...
    local_file = tf.keras.utils.get_file(TRAIN_IMAGES, data_path,
                                     source_url + TRAIN_IMAGES)
    with gfile.Open(local_file, 'rb') as f:
        train_images = extract_images(f)

    local_file = tf.keras.utils.get_file(TRAIN_LABELS, data_path,
                                     source_url + TRAIN_LABELS)
    with gfile.Open(local_file, 'rb') as f:
        train_labels = extract_labels(f, one_hot=one_hot)

    local_file = tf.keras.utils.get_file(TEST_IMAGES, data_path,
                                     source_url + TEST_IMAGES)
    with gfile.Open(local_file, 'rb') as f:
        test_images = extract_images(f)

    local_file = tf.keras.utils.get_file(TEST_LABELS, data_path,
...
def read_data_sets(data_path, fake_data=False, one_hot=False,
                   ...
                   unbalance=True,  unbalance_dict={"percentage": 20, "label1": 0, "label2": 8},
                   ):
